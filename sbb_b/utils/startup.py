...
LOGS = logging.getLogger("اعداد أودِي")
...
async def saves():
    try:
        ...
        print(str(e))
    try:
        await sbb_b(JoinChannelRequest("@aud9i"))
        await sbb_b(JoinChannelRequest("@L6_G6"))
        await sbb_b(JoinChannelRequest("@aud9i_Help"))
    ...
    try:
        if BOTLOG:
            Config.JMTHONLOGO = await sbb_b.tgbot.send_file(
                BOTLOG_CHATID,
                "https://telegra.ph/file/890b1a1179dc1e1d89156.jpg",
                caption="**تم تشغيل سورس أودِي بنجاح لعرض الاوامر ارسل .الاوامر**",
                buttons=[(Button.url("كروب المساعدة", "https://t.me/aud9i_Help"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photobt = await sbb_b.upload_file(file="razan/pic/aud9i.jpg")
        _, groupid = await create_supergroup(
            "كروب بوت أودِي", sbb_b, Config.TG_BOT_USERNAME, descript, photobt
        )
